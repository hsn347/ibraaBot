...
def reset_SUM():
    global CURRENT_DEVICE , SUM_manager , attempt , SCROLL_ICONS
...
def Clean_fast(device, target_icons: list = None, custom_actions: dict = None, max_attempts: int = 10) -> bool:
    ...
    for attempt in range(max_attempts):
        if attempt >= max_attempts - 1:
            ...
            if 'SUM_manager' in globals() and SUM_manager is not None:
                return SUM_manager.stop_execution()
            return False
...
def step_1(device):
    global SCROLL_ICONS
    if len(SCROLL_ICONS) == 0:
        Clean_fast(device)
        return SUM_manager.stop_execution()

    result_SCROLL_ICONS = find_multiple_icons(device , SCROLL_ICONS ,screen_region=(0, 100, 720, 1055) , timeout=2.5 ,threshold=0.7)
    if result_SCROLL_ICONS: 
        icon_path1, coordinates = result_SCROLL_ICONS
        click_coordinates(device , coordinates[0] , coordinates[1])
        SCROLL_ICONS.remove(icon_path1)
        time.sleep(0.2)
        return SUM_manager.go_to_step_and_continue(1)

    scroll_device(device, 230, 700, 230, 120 , 600)
    time.sleep(1)
...
def step_2(device):
    ...
    result_SCROLL_ICONS = find_multiple_icons(device , SCROLL_ICONS ,screen_region=(0, 100, 640, 1055) , timeout=2.5 ,threshold=0.7)
    if result_SCROLL_ICONS: 
        icon_path1, coordinates = result_SCROLL_ICONS
        click_coordinates(device , coordinates[0] , coordinates[1])
        SCROLL_ICONS.remove(icon_path1)
        time.sleep(0.2)
        return SUM_manager.go_to_step_and_continue(2)

    scroll_device(device, 600, 800, 120, 800 , 600)
    time.sleep(1)
...
def step_3(device):
    ...
    if result_SCROLL_ICONS: 
        ...
        SCROLL_ICONS.remove(icon_path1)
        time.sleep(0.2)
        return SUM_manager.go_to_step_and_continue(3)

    scroll_device(device, 520, 475, 200, 925 , 600)
    time.sleep(1)
...
def step_4(device):
    ...
    if result_SCROLL_ICONS: 
        ...
        SCROLL_ICONS.remove(icon_path1)
        time.sleep(0.2)
        return SUM_manager.go_to_step_and_continue(4)

    scroll_device(device, 130, 400, 560, 540 , 600)
    time.sleep(1)
...
def step_5(device):
    ...
    if result_SCROLL_ICONS: 
        ...
        SCROLL_ICONS.remove(icon_path1)
        time.sleep(0.2)
        return SUM_manager.go_to_step_and_continue(5)


SUM_manager = None 

def run_SUM_stage(device_id: str = None):

    global SUM_manager , CURRENT_DEVICE
    
    try:
        if SUM_manager is None or (device_id and (SUM_manager.device_id != device_id)):
            SUM_manager = TroopsManager6(device_id or DEVICE_ID)
            CURRENT_DEVICE = device_id or DEVICE_ID
            
            SUM_manager.add_step(1, "فتح قائمة القوات", step_1, "فتح قائمة القوات من القائمة الرئيسية")
            SUM_manager.add_step(2, "فتح قائمة القوات", step_2, "فتح قائمة القوات من القائمة الرئيسية")
            SUM_manager.add_step(3, "فتح قائمة القوات", step_3, "فتح قائمة القوات من القائمة الرئيسية")
            SUM_manager.add_step(4, "فتح قائمة القوات", step_4, "فتح قائمة القوات من القائمة الرئيسية")
            SUM_manager.add_step(5, "فتح قائمة القوات", step_5, "فتح قائمة القوات من القائمة الرئيسية")

        SUM_manager.execute_all_steps()
