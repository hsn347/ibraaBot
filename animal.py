...
def reset_animal():
    global outflow_import , outflow_import2 , CURRENT_DEVICE , animal_manager , attempt
...
def step_5_Book_Loot(device):
    ...
    animal_index = animal_flag.index(animal)
...
def step_7_choose_animal(device):
    result_Choose_Goods = wait_for_icon_coordinates(device,"image/patrolbtn.png",screen_region=(200, 1150, 520, 1270) , timeout=3)
    if result_Choose_Goods:
        x , y = result_Choose_Goods
        click_coordinates(device , x , y )
        time.sleep(0.3)
        Clean_fast(device)
        return animal_manager.stop_execution()
