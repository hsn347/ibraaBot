...
def reset_Treasure():
    global CURRENT_DEVICE , treasure_manager , outflow_import , outflow_import2 , attempt

    outflow_import = 0 
    outflow_import2 = 0
    attempt = 0
    CURRENT_DEVICE = None
    treasure_manager = None
...
def Clean_fast(device, target_icons: list = None, custom_actions: dict = None, max_attempts: int = 10) -> bool:
    ...
    for attempt in range(max_attempts):
        if attempt >= max_attempts - 1:
            ...
            if 'treasure_manager' in globals() and treasure_manager is not None:
                return treasure_manager.stop_execution()
            return False
...
def step_1_Clean_fast(device):
    Clean_fast(device)
    result_Treasure = wait_for_icon_coordinates(device, "image/Treasure.png"  ,screen_region=(150, 300, 550, 700),timeout=2)
    if result_Treasure:
        x , y = result_Treasure
        click_coordinates(device , x , y)

        result_Cliam = wait_for_icon_coordinates(device, "image/tr.png"  ,screen_region=(150, 700, 555, 850),timeout=2)
        if result_Cliam:
            x , y = result_Cliam
            click_coordinates(device , x , y)
            return treasure_manager.stop_execution()
        else:
            click_coordinates(device , 442 , 1242)
            return treasure_manager.stop_execution()
    else:
        return True
    
def step_2_Clean_fast(device):
...
def step_4_Result_Loot(device):
    ...
    scroll_device(device, 250, 1000, 250, 600 , 500)
    time.sleep(1)
    click_coordinates(device , 250 , 700)
    time.sleep(0.4)
    result_loot_1 = wait_for_icon_coordinates_custom_thresholds(device ,"image/Port.png" ,screen_region=(0, 100, 250, 1280) , timeout=1.5 ,thresholds=[0.7 ,0.8])
    if result_loot_1:
        x , y = result_loot_1 
        ...
        return treasure_manager.go_to_step_and_continue(3)
...
def run_treasure_stage(device_id: str = None):

    global treasure_manager , CURRENT_DEVICE
    
    try:
        if treasure_manager is None or (device_id and (treasure_manager.device_id != device_id)):
            treasure_manager = TroopsManager(device_id or DEVICE_ID)
            CURRENT_DEVICE = device_id or DEVICE_ID

            treasure_manager.add_step(1, "فتح قائمة القوات", step_1_Clean_fast, "فتح قائمة القوات من القائمة الرئيسية")
            treasure_manager.add_step(2, "فتح قائمة القوات", step_2_Clean_fast, "فتح قائمة القوات من القائمة الرئيسية")
            treasure_manager.add_step(3, "فتح قائمة القوات", step_3_open_loot_menu, "فتح قائمة القوات من القائمة الرئيسية")
            treasure_manager.add_step(4, "فتح قائمة القوات", step_4_Result_Loot, "فتح قائمة القوات من القائمة الرئيسية")
            treasure_manager.add_step(5, "فتح قائمة القوات", step_5_C, "فتح قائمة القوات من القائمة الرئيسية")
            

        treasure_manager.reset_legions_index()
        
        treasure_manager.execute_all_steps()
