...
def reset_Hammer():
    global CURRENT_DEVICE , Hammer_manager , attempt
    attempt = 0
    CURRENT_DEVICE = None
    Hammer_manager = None
...
def Clean_fast(device, target_icons: list = None, custom_actions: dict = None, max_attempts: int = 10) -> bool:
    ...
    for attempt in range(max_attempts):
        if attempt >= max_attempts - 1:
            ...
            if 'Hammer_manager' in globals() and Hammer_manager is not None:
                return Hammer_manager.stop_execution()
            return False
...
def step_1_Harvest(device):
    result_Hammer = wait_for_icon_coordinates(device, "image/Hammer.png" ,screen_region=(480, 900, 720, 1280),timeout=2)
    if result_Hammer:
        x , y = result_Hammer
        click_coordinates(device , x , y)
        time.sleep(0.3)
        result_Harvest = wait_for_icon_coordinates(device, "image/Harvest.png" ,screen_region=(0, 300, 720, 675),timeout=3)
        if result_Harvest:
            x , y = result_Harvest
            click_coordinates(device , x , y)
            time.sleep(0.5)
            click_coordinates(device , 360 , 1000)
            time.sleep(2)
        click_coordinates(device , 448 , 1250)
        return True
    else:
        Clean_fast(device)
        return Hammer_manager.stop_execution()
    
def step_2_Collect(device):
    result_Hammer = wait_for_icon_coordinates(device, "image/Hammer.png" ,screen_region=(480, 900, 720, 1280),timeout=2)
    if result_Hammer:
        x , y = result_Hammer
        click_coordinates(device , x , y)
        time.sleep(0.2)
        result_Harvest = wait_for_icon_coordinates(device, "image/Collect.png" ,screen_region=(0, 300, 720, 675),timeout=2)
        if result_Harvest:
            x , y = result_Harvest
            click_coordinates(device , x , y)
            time.sleep(0.5)
...
def run_Hammer_stage(device_id: str = None):

    global Hammer_manager , CURRENT_DEVICE
    
    try:
        if Hammer_manager is None or (device_id and (Hammer_manager.device_id != device_id)):
            Hammer_manager = TroopsManager(device_id or DEVICE_ID)
            CURRENT_DEVICE = device_id or DEVICE_ID

            Hammer_manager.add_step(1, "فتح قائمة القوات", step_1_Harvest, "فتح قائمة القوات من القائمة الرئيسية")
            Hammer_manager.add_step(2, "فتح قائمة القوات", step_2_Collect, "فتح قائمة القوات من القائمة الرئيسية")
            Hammer_manager.add_step(3, "فتح قائمة القوات", step_3_LORD2, "فتح قائمة القوات من القائمة الرئيسية")
        
        Hammer_manager.execute_all_steps()
